...
def load_model():
    try:
        ...
        sess = ort.InferenceSession('churn_ann_model.onnx')
        with open('scaler.pkl','rb') as f: scaler = pickle.load(f)
        with open('feature_columns.pkl','rb') as f: cols = pickle.load(f)
        return sess, scaler, cols, True, ""
...
def predict_onnx(sess, X):
    ...
    return float(sess.run(None, {inp: X})[0][0][0])
...
with st.sidebar:
    ...
    st.markdown('<p style="text-align:center;font-size:0.65rem;color:#8b949e!important;">ANN · ONNX Runtime · SMOTE<br>Streamlit · sklearn · pandas</p>', unsafe_allow_html=True)
...
<div class="hero">
  ...
  <span class="badge">☁️ Streamlit Cloud</span>
...
with tab4:
    ...
    with c1:
        st.markdown("""<div class="card"><div class="card-title">🧠 Model Architecture</div>
          ...
          <tr><td>Cloud Format</td><td>ONNX (runs without TensorFlow)</td></tr>
          </table></div>""", unsafe_allow_html=True)
    with c2:
        st.markdown('<div class="card"><div class="card-title">📁 Files in This Repo</div>', unsafe_allow_html=True)
        st.code("""OPTION_A_StreamlitCloud/
├── app.py                   ← This app
├── requirements.txt         ← Dependencies
├── churn_ann_model.onnx     ← ANN model (ONNX)
